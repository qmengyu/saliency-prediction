...
def corr_coeff_loss(pred, target):
    size = pred.size()
    new_size = (-1, size[-1] * size[-2])
    pred = pred.reshape(new_size)
...
def kld_loss(pred, target):
    ...
    # return loss
    eps = 2.2204e-16
    pred_map = pred/torch.sum(pred)
    gt_map = target/torch.sum(target)
...
def loss_similarity(pred_map, gt_salmap):
    gt_salmap = (gt_salmap - torch.min(gt_salmap)) / (torch.max(gt_salmap) - torch.min(gt_salmap))
    gt_salmap = gt_salmap / torch.sum(gt_salmap)

    pred_map = (pred_map - torch.min(pred_map)) / (torch.max(pred_map) - torch.min(pred_map))
    pred_map = pred_map / torch.sum(pred_map)

    diff = torch.min(gt_salmap, pred_map)
